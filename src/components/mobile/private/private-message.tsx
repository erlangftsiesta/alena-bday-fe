...
import { Heart, Play, Pause, X, Sparkles, Lock, Globe } from 'lucide-react'
import { usePrivateMessages } from "../../../hooks/usePrivateMessage"

export default function PrivateMessagesMobile() {
  const {
    messages,
    loading,
    error,
  ...
  const handleMakePublic = async (messageId: string) => {
    const success = await makeMessagePublic(messageId)
    if (success) {
      closeModal()
    }
  }
    ...
    return (
      <div className="min-h-screen bg-gradient-to-br from-pink-100 via-orange-50 to-pink-200 flex items-center justify-center">
        <div className="text-center">
          ...
          <p className="text-pink-600 font-medium">Loading private messages... 🔒</p>
        </div>
      </div>
    )
  }
    ...
    <div className="min-h-screen bg-gradient-to-br from-pink-100 via-orange-50 to-pink-200 p-4">
      ...
      <div className="text-center mb-6">
        <div className="flex items-center justify-center gap-2 mb-2">
          ...
          <h1 className="text-2xl font-bold text-pink-800">Private Love Messages</h1>
          <Lock className="w-6 h-6 text-pink-500" />
        </div>
        <p className="text-pink-600 text-sm">Your secret musical treasures 🔒💕</p>
      </div>
          ...
          >
            <div className="flex items-center gap-3">
              ...
              <div className="flex-1 min-w-0">
                ...
                >
                  <p className="text-pink-700 text-sm leading-relaxed mb-2">{truncateMessage(message.message, 60)}</p>
      ...
      {messages.length === 0 && !loading && (
        <div className="text-center py-12">
          <Lock className="w-16 h-16 text-pink-300 mx-auto mb-4" />
          <p className="text-pink-600 text-lg font-medium">No private messages yet</p>
          <p className="text-pink-500 text-sm">Your secret messages will appear here 🔒</p>
        </div>
      )}
        ...
        <div className="fixed inset-0 bg-black/50 backdrop-blur-sm z-50 flex items-end">
          <div className="bg-white rounded-t-3xl w-full max-h-[80vh] overflow-hidden animate-slide-up">
            ...
            <div className="p-6">
              ...
              <div className="space-y-4">
                <div>
                  <h3 className="text-pink-800 font-semibold mb-2">Private Message:</h3>
                ...
                <div className="pt-4">
                  <button
                    onClick={() => handleMakePublic(selectedMessage.id)}
                    disabled={loading}
                    className="w-full bg-gradient-to-r from-orange-400 to-pink-400 text-white py-3 rounded-xl font-medium shadow-lg hover:shadow-xl transition-all duration-200 disabled:opacity-50 flex items-center justify-center gap-2"
                  >
                    <Globe className="w-4 h-4" />
                    {loading ? "Making Public..." : "Make This Message Public 🌍"}
                  </button>
                </div>
              </div>
            </div>
